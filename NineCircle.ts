...
import { NineCirclesLevel, difficultyRanks, lengthRanks, APIInformation, Colour } from "./NineCirclesLevel.js";
import { GuessResults, RelativeGuess } from "./Guess.js";
import * as gd from "gj-boomlings-api";

function evaluateColours(guessed: [Colour, Colour], correct: [Colour, Colour]): [boolean, boolean] {
    if ((guessed === correct) || (guessed.reverse() === correct)) {
        return [true, true];
    }

    let firstColourIndex: number = correct.indexOf(guessed[0]);
    let secondColourIndex: number;
    if (firstColourIndex === -1) {
        secondColourIndex = correct.indexOf(guessed[1]);
    } else {
        let otherIndex: number = firstColourIndex === 0 ? 1 : 0;
...
function evaluateValues(guessed: number, correct: number): RelativeGuess {
    if (guessed === correct) { return RelativeGuess.CORRECT; }
...
class NineCircle {
    ...
    async guessLevel(levelName: string): Promise<GuessResults> {
        ...
        let levelId: number = nameToIdObj[levelName];
        let guessedInformation: APIInformation = await getLevelInformation(levelId);

        return this.getLevelResults(getNineCirclesLevel(levelId), guessedInformation);
    }

    private getLevelResults(non_api: NineCirclesLevel, api: APIInformation): GuessResults {
        return {
